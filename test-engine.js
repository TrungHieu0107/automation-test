  ...
  async runTests(testScenarioPath) {
    try {
      if (testScenarioPath.endsWith(".xml")) {
        ...
        testScenarioPath.endsWith(".yaml") ||
        testScenarioPath.endsWith(".yml")
      ) {
  ...
  async executeTestHierarchy(testNode, baseDir, level, skipNavigation = false) {
    ...
    try {
      ...
      this.logger.logError(
        `${indent}  Failed to load test file: ${testFile}`,
        error,
      );
      return false;
    }
    ...
    if (testPassed && testNode.test) {
      const childTests = Array.isArray(testNode.test)
        ? testNode.test
        : [testNode.test];
      this.logger.log(
        `${indent}  → Executing ${childTests.length} child test(s)...`,
      );

      for (const childTest of childTests) {
        const childPassed = await this.executeTestHierarchy(
          childTest,
          baseDir,
          level + 1,
          true,
