...
class ActionHandler {
	...
	async executeStepWithDialog(clickStep, dialogStep, indent = '') {
		...
		const clickPromise = element.click({ timeout: this.config.execution.actionTimeout })
		...
		try {
			...
			try {
				await clickPromise
				throw new Error(`Expected ${dialogStep.dialogType} dialog did not appear after clicking`)
			} catch (clickErr) {
				// Click also failed
				throw new Error(`Click failed and dialog did not appear: ${clickErr.message}`)
			}
		}
	...
	async executeSubmit(submitAction, indent = '') {
		...
		if (submitAction.dialogs && Array.isArray(submitAction.dialogs)) {
			await this.executeStepWithMultipleDialogs(step, submitAction.dialogs, indent)

			if (submitAction.postSubmitWait) {
				this.logger.log(`${indent}  ⏳ Waiting ${submitAction.postSubmitWait}ms after submit...`)
	...
	async executeAssertion(assertion, indent = '') {
		...
		switch (assertion.type) {
			...
			case 'class':
				...
				if (assertion.hasClass) {
					...
					this.logger.log(`${indent}  ✓ Element has class "${assertion.hasClass}"`)
					return
				}
				break
			...
			case 'visible':
				...
				this.logger.log(`${indent}  ✓ Element is ${isVisible ? 'visible' : 'hidden'} as expected`)
				return
			...
			case 'enabled':
				...
				this.logger.log(`${indent}  ✓ Element is ${isEnabled ? 'enabled' : 'disabled'} as expected`)
				return
		...
		const actualTrimmed = typeof actualValue === 'string' ? actualValue.trim() : String(actualValue)
		const expectedTrimmed =
			typeof expectedValue === 'string' ? expectedValue.trim() : String(expectedValue)
	...
	async executeStepWithMultipleDialogs(clickStep, dialogSteps, indent = '') {
		this.logger.log(`${indent}  🔔 Will handle ${dialogSteps.length} sequential dialogs...`)
		...
		const dialogHandler = async (dialog) => {
			...
			if (dialogStep.action === 'accept') {
				...
				await dialog.dismiss()
				this.logger.log(`${indent}  ❌ Dialog ${dialogCount} dismissed (Cancel)`)
			} else {
				throw new Error(`Unknown dialog action: ${dialogStep.action}`)
			}
		}
		...
		try {
			...
			const shouldWaitForNavigation =
				clickStep.waitForNavigation || dialogSteps.some((d) => d.waitForNavigation)
			...
			while (dialogCount < totalDialogs && Date.now() - startTime < maxWaitTime) {
				await this.wait(100)
			}

			if (dialogCount < totalDialogs) {
				throw new Error(`Only ${dialogCount}/${totalDialogs} dialogs were handled`)
			}
			...
			if (shouldWaitForNavigation) {
				try {
					...
					this.logger.log(`${indent}  🧭 Navigation already completed or not required`)
				}

				// Use postNavigationWait from any dialog step that has it
				const postNavigationWait = dialogSteps.find((d) => d.postNavigationWait)?.postNavigationWait
				if (postNavigationWait) {
					await this.wait(postNavigationWait)
				}
			}
