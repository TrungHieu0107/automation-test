...
class ReportGenerator {
  ...
  async generate(testResults) {
    const report = {
      ...
      results: testResults.map(result => ({
        ...result,
        screenshots: (result.screenshots || []).filter(s => s && (typeof s === 'string' || s.path))
      })),
    };
  ...
  groupScreenshots(screenshots) {
    ...
    screenshots.forEach(screenshot => {
      ...
      const screenshotObj = typeof screenshot === 'string'
              ...
              <div class="screenshots">
                ${screenshotGroups.submitSteps.length > 0 ? `
                  <div class="screenshot-section">
                    ...
                    <div class="screenshot-timeline">
                      ${screenshotGroups.submitSteps.map((ss, ssIdx) => {
                  const screenshotPath = typeof ss === 'string' ? ss : ss.path;
                  const stepName = typeof ss === 'object' && ss.step ? ss.step : `Step ${ssIdx + 1}`;
                  const timestamp = typeof ss === 'object' && ss.timestamp ? new Date(ss.timestamp).toLocaleTimeString() : '';
                ...
                ${screenshotGroups.dialogs.length > 0 ? `
                  <div class="screenshot-section">
                    ...
                    <div class="screenshot-grid">
                      ${screenshotGroups.dialogs.map(ss => {
                    const screenshotPath = typeof ss === 'string' ? ss : ss.path;
                ...
                ${screenshotGroups.regular.length > 0 ? `
                  <div class="screenshot-section">
                    ...
                    <div class="screenshot-grid">
                      ${screenshotGroups.regular.map(ss => {
                      const screenshotPath = typeof ss === 'string' ? ss : ss.path;
                      const stage = this.getScreenshotStage(ss);
                      return `
                        <div class="screenshot-item" onclick="viewImage('${screenshotPath.replace(/\\/g, "/")}')">
