...
class YamlValidator {
  constructor() {
    this.validActionTypes = ['input', 'click', 'checkbox', 'radio', 'select', 'select_radio', 'dropdown', 'check', 'dialog'];
  ...
  validateScenario(scenarioData) {
    ...
    if (scenarioData.verify || scenarioData.assertions) {
      this.validateVerifications(scenarioData.verify || scenarioData.assertions);
    }
  ...
  validateTestList(listData) {
    ...
    if (!testList) {
      throw new ValidationError('Test list must contain "test_list" or "tests" field');
    }

    if (!Array.isArray(testList)) {
      throw new ValidationError('Test list must be an array', 'test_list', typeof testList);
    }
    ...
    if (typeof item === 'object' && item !== null) {
      ...
      if (item.file) {
        ...
        if (typeof item.file !== 'string') {
          throw new ValidationError(`Test list item ${index}: 'file' must be a string`, `tests[${index}].file`);
        }
      } else {
        // Format: { "path": [...children] }
        const keys = Object.keys(item);
        if (keys.length !== 1) {
          throw new ValidationError(`Test list item ${index}: must have exactly one file path as key`, `tests[${index}]`);
        }
      }
      return;
    }

    throw new ValidationError(`Test list item ${index}: must be a string or object`, `tests[${index}]`, typeof item);
  }
  ...
  validateTestName(scenarioData) {
    ...
    if (!name) {
      throw new ValidationError('Test scenario must have a "name" or "test_name" field');
    }

    if (typeof name !== 'string') {
      throw new ValidationError('Test name must be a string', 'name', typeof name);
    }
  ...
  validateUrl(url) {
    ...
    if (!url.startsWith('http://') && !url.startsWith('https://')) {
      throw new ValidationError('URL must start with http:// or https://', 'url', url);
    }
  }
  ...
  validateSteps(steps) {
    ...
    if (!Array.isArray(steps)) {
      throw new ValidationError('Steps must be an array', 'steps', typeof steps);
    }

    if (steps.length === 0) {
      throw new ValidationError('Steps array cannot be empty - at least one action is required');
    }
  ...
  validateStep(step, index) {
    if (!step || typeof step !== 'object') {
      throw new ValidationError(`Step ${index + 1}: must be an object`, `steps[${index}]`, typeof step);
    }

    const actionTypes = Object.keys(step);
    
    if (actionTypes.length === 0) {
      throw new ValidationError(`Step ${index + 1}: must have an action type (e.g., input, click, checkbox)`, `steps[${index}]`);
    }

    if (actionTypes.length > 1) {
      throw new ValidationError(`Step ${index + 1}: must have exactly one action type, found: ${actionTypes.join(', ')}`, `steps[${index}]`);
    }
  ...
  validateActionData(actionType, data, stepIndex) {
    if (!data) {
      throw new ValidationError(`Step ${stepIndex + 1}: ${actionType} action requires data`, `steps[${stepIndex}].${actionType}`);
    }
  ...
  validateInputAction(data, stepIndex) {
    ...
    if (typeof data !== 'object') {
      throw new ValidationError(`Step ${stepIndex + 1}: input action data must be an object`, `steps[${stepIndex}].input`);
    }

    // Must have a selector
    const hasSelector = data.field || data.name || data.id || data.selector;
    if (!hasSelector) {
      throw new ValidationError(`Step ${stepIndex + 1}: input action must have a selector (field, name, id, or selector)`, `steps[${stepIndex}].input`);
    }

    // Must have a value
    if (data.value === undefined) {
      throw new ValidationError(`Step ${stepIndex + 1}: input action must have a "value" field`, `steps[${stepIndex}].input`);
    }
  ...
  validateClickAction(data, stepIndex) {
    ...
    if (typeof data !== 'object') {
      throw new ValidationError(`Step ${stepIndex + 1}: click action data must be a string or object`, `steps[${stepIndex}].click`);
    }

    const hasSelector = data.button || data.element || data.xpath || data.selector;
    if (!hasSelector) {
      throw new ValidationError(`Step ${stepIndex + 1}: click action must have a selector (button, element, xpath, or selector)`, `steps[${stepIndex}].click`);
    }
  ...
  validateCheckboxAction(data, stepIndex) {
    if (typeof data !== 'object') {
      throw new ValidationError(`Step ${stepIndex + 1}: checkbox action data must be an object`, `steps[${stepIndex}].checkbox`);
    }

    const hasSelector = data.field || data.name || data.id || data.selector;
    if (!hasSelector) {
      throw new ValidationError(`Step ${stepIndex + 1}: checkbox action must have a selector`, `steps[${stepIndex}].checkbox`);
    }

    if (data.value !== undefined && typeof data.value !== 'boolean' && typeof data.value !== 'string') {
      throw new ValidationError(`Step ${stepIndex + 1}: checkbox value must be a boolean or string`, `steps[${stepIndex}].checkbox.value`);
    }
  }
  ...
  validateRadioAction(data, stepIndex) {
    if (typeof data !== 'object') {
      throw new ValidationError(`Step ${stepIndex + 1}: radio action data must be an object`, `steps[${stepIndex}].radio`);
    }

    const hasSelector = data.xpath || data.id || data.name || data.selector;
    if (!hasSelector) {
      throw new ValidationError(`Step ${stepIndex + 1}: radio action must have a selector`, `steps[${stepIndex}].radio`);
    }
  }
  ...
  validateSelectAction(data, stepIndex) {
    if (typeof data !== 'object') {
      throw new ValidationError(`Step ${stepIndex + 1}: select action data must be an object`, `steps[${stepIndex}].select`);
    }

    const hasSelector = data.field || data.name || data.id || data.selector;
    if (!hasSelector) {
      throw new ValidationError(`Step ${stepIndex + 1}: select action must have a selector`, `steps[${stepIndex}].select`);
    }

    if (data.value === undefined) {
      throw new ValidationError(`Step ${stepIndex + 1}: select action must have a "value" field`, `steps[${stepIndex}].select`);
    }
  ...
  validateDialogAction(data, stepIndex) {
    if (typeof data !== 'object') {
      throw new ValidationError(`Step ${stepIndex + 1}: dialog action data must be an object`, `steps[${stepIndex}].dialog`);
    }
  ...
  validateSubmit(submit) {
    ...
    if (typeof submit !== 'object') {
      throw new ValidationError('Submit must be an object', 'submit', typeof submit);
    }

    const hasAction = submit.button || submit.click || submit.selector;
    if (!hasAction) {
      throw new ValidationError('Submit must have a button, click, or selector field', 'submit');
    }

    // Validate wait times if present
    if (submit.wait_after || submit.waitAfter || submit.postSubmitWait) {
      const waitValue = submit.wait_after || submit.waitAfter || submit.postSubmitWait;
  ...
  validateWaitTime(value, fieldPath) {
    if (typeof value === 'number') {
      if (value < 0) {
        throw new ValidationError(`Wait time must be positive`, fieldPath, value);
      }
      return;
    }
    ...
    throw new ValidationError(`Wait time must be a number or string`, fieldPath, typeof value);
  }
  ...
  validateVerifications(verifications) {
    if (!Array.isArray(verifications)) {
      throw new ValidationError('Verifications must be an array', 'verify', typeof verifications);
    }
  ...
  validateVerification(verification, index) {
    ...
    if (typeof verification !== 'object') {
      throw new ValidationError(`Verification ${index + 1}: must be a string or object`, `verify[${index}]`);
    }

    // Check for known verification types
    const hasType = verification.message || verification.input_value || verification.style || 
                    verification.visible !== undefined || verification.hidden !== undefined ||
                    verification.has_class || verification.attribute;
